...
// Rebuild the category picker showing only categories matching the transaction type.
// type: 'expense' → only 'despesa'; 'income' → only 'receita'; others → all categories.
function buildCatPickerForType(type) {
  const dd = document.getElementById('catPickerDropdown');
  if (!dd || !state.categories) return;

  const catTypeFilter = type === 'expense' ? 'despesa' : type === 'income' ? 'receita' : null;
  const cats = catTypeFilter
    ? state.categories.filter(c => c.type === catTypeFilter)
    : state.categories;
  ...
  parents.forEach(p => {
    ...
    if (hasChildren) {
      html += `<div class="cat-children" id="catGroup-${p.id}">`;
      children.forEach(c => {
  ...
  // If currently selected category doesn't match the new filter, clear it
  if (catTypeFilter) {
    const currentId = document.getElementById('txCategoryId')?.value;
    if (currentId) {
      const cat = state.categories.find(c => c.id === currentId);
      if (cat && cat.type !== catTypeFilter) setCatPickerValue(null);
    }
  }
}
